# GameHub — Game Store Web Application

A full-stack web application for a game store built with **HTML/CSS/JS** on the frontend and **Node.js + Express + MySQL** on the backend. The frontend and backend run on **separate web servers** as required by the project specification.

---

> [!IMPORTANT]
> **GitHub Classroom Team Notice**
>
> According to the submission instructions, the team member with the **lowest student ID (6788004)** should be the one who creates the GitHub Classroom team. However, in our group, the team was created by student **6788121 (Numning Sungkagul)** due to a misunderstanding at the time of submission. We apologize for the deviation and respectfully bring this to the instructor's attention.

---

## Project Structure

```
WebDev_Phase2/
├── sec2_gr12_fe_src/          # Frontend server & static files (HTML, CSS, JS)
│   ├── server.js              # Express server with clean URL routing (port 5500)
│   ├── css/style.css          # Global stylesheet (~77KB, ~3200 lines)
│   ├── js/                    # Page-specific JavaScript files
│   │   ├── home.js            # Homepage logic (carousels, featured, news)
│   │   ├── search.js          # Game search with multi-criteria filtering
│   │   ├── detail.js          # Game detail page (gallery, reviews, wishlist)
│   │   ├── cart.js            # Shopping cart management
│   │   ├── wishlist.js        # Wishlist functionality
│   │   ├── admin.js           # Admin dashboard (CRUD operations)
│   │   ├── login.js           # Admin login authentication
│   │   ├── user-login.js      # User registration & login
│   │   └── team.js            # Team member cards
│   ├── images/                # Local static images
│   │   ├── games/             # Game cover images (front.jpg) and screenshots (detail_N.jpg)
│   │   └── team/              # Team member profile photos (<studentId>.jpg)
│   ├── index.html             # Home page
│   ├── search.html            # Browse/Search games page
│   ├── detail.html            # Game detail page
│   ├── cart.html              # Shopping cart page
│   ├── wishlist.html          # Wishlist page
│   ├── user-login.html        # User sign in / register page
│   ├── team.html              # Team members page
│   ├── login.html             # Admin login page
│   └── admin.html             # Admin dashboard page
│
├── sec2_gr12_ws_src/          # Backend API server (Node.js + Express)
│   ├── server.js              # Main Express server entry point (port 3000)
│   ├── config/
│   │   └── db.js              # MySQL connection pool + database initialization
│   ├── sec2_gr12_database.sql # Database schema with 10 tables + seed data
│   ├── routes/                # Express route modules
│   │   ├── games.js           # Game CRUD endpoints (search, insert, update, delete)
│   │   ├── auth.js            # Admin authentication endpoint
│   │   ├── reviews.js         # Review endpoints (get, post, stats, delete)
│   │   ├── wishlist.js        # Wishlist endpoints (get, add, remove, check)
│   │   ├── cart.js            # Cart endpoints (get, add, update, remove, clear)
│   │   ├── users.js           # User registration & login endpoints
│   │   └── news.js            # Public API proxy (FreeToGame API)
│   ├── .env                   # Environment variables (edit with your MySQL credentials)
│   └── package.json           # Backend dependencies
│
├── .gitignore                 # Git ignore rules (node_modules, .env)
├── REPORT_PART1.md            # Project report — Part 1
├── REPORT_PART2.md            # Project report — Part 2
└── README.md                  # This file
```

---

## Team Members

| Student ID | Name                     | Role               |
| ---------- | ------------------------ | ------------------ |
| 6788004    | Promdpoori Athipornwanit | Frontend Developer |
| 6788121    | Numning Sungkagul        | Backend Developer  |
| 6788134    | Kasidech Thongpakdee     | Database Designer  |
| 6788181    | Nannalin Leelaparung     | UI/UX Designer     |

---

## Prerequisites

- [Node.js](https://nodejs.org/) v14 or higher
- [MySQL](https://www.mysql.com/) server installed locally and running (tested with MySQL 9.7)

---

## How to Run

### 1. Configure Database Connection

Edit the `.env` file inside the `sec2_gr12_ws_src/` folder with your MySQL credentials:

```env
# Database Configuration
DB_HOST=localhost
DB_PORT=3306
DB_USER=root
DB_PASSWORD=your_mysql_password_here
DB_NAME=game_store

# Server Configuration
PORT=3000
```

> **Note:** Update `DB_PASSWORD` to match your local MySQL root password before starting the server.

---

### 2. Start the Backend Server (Port 3000)

```bash
cd sec2_gr12_ws_src
npm install
npm start
```

The backend server will **automatically**:

1. Create the `game_store` database if it does not exist.
2. Create all 10 required tables and insert seed data (24 games, genres, admins, reviews, users) on the first run.
3. Start the Express API server on port **3000**.

---

### 3. Start the Frontend Server (Port 5500)

Open a **new terminal** and run:

```bash
cd sec2_gr12_fe_src
npm install
npm start
```

The frontend server serves all HTML/CSS/JS files on port **5500** with clean URL routing.

---

### 4. Open in Browser

Visit **http://localhost:5500** to access the application.

> **Important:** Both servers must be running simultaneously. The frontend makes all API calls to the backend at `http://localhost:3000/api/`.

---

### ⚠️ Resetting the Database

If your machine already has an older version of the `game_store` database and the backend skips re-importing, follow these steps to force a full fresh install:

1. Open `sec2_gr12_ws_src/.env`
2. Add the line: `DB_RESET=true`
3. Restart the backend with `npm start`
4. The server will **drop and recreate** the database with the latest schema and seed data
5. **Remove `DB_RESET=true`** immediately after — leaving it in will wipe all data on every restart

```env
# DB_RESET=true   ← uncomment temporarily, then remove after first restart
```

---

## Pages (Frontend)

| Page            | Clean URL                         | HTML File         |
| --------------- | --------------------------------- | ----------------- |
| Home            | http://localhost:5500/home        | `index.html`      |
| Browse Games    | http://localhost:5500/games       | `search.html`     |
| Game Detail     | http://localhost:5500/game?id=1   | `detail.html`     |
| Shopping Cart   | http://localhost:5500/cart        | `cart.html`       |
| Wishlist        | http://localhost:5500/wishlist    | `wishlist.html`   |
| User Account    | http://localhost:5500/account     | `user-login.html` |
| Team            | http://localhost:5500/team        | `team.html`       |
| Admin Dashboard | http://localhost:5500/admin       | `admin.html`      |
| Admin Login     | http://localhost:5500/admin/login | `login.html`      |

> Old `.html` URLs (e.g. `/index.html`) automatically redirect with HTTP 301 to clean URLs (e.g. `/home`).

---

## Default Login Credentials

### Admin Accounts

| Username    | Password   | Role            |
| ----------- | ---------- | --------------- |
| `admin`     | `admin123` | Super Admin     |
| `sarah`     | `admin123` | Manager         |
| `mike`      | `admin123` | Manager         |
| `lisa`      | `admin123` | Manager         |
| `tom`       | `admin123` | Manager         |
| `emma`      | `admin123` | Manager         |
| `james`     | `admin123` | Manager         |
| `lucas`     | `admin123` | Manager         |
| `mia`       | `admin123` | Manager         |
| `sophia`    | `admin123` | Moderator       |
| `oliver`    | `admin123` | Moderator       |
| `isabella`  | `admin123` | Moderator       |
| `noah`      | `admin123` | Moderator       |
| `olivia`    | `admin123` | Moderator       |
| `ethan`     | `admin123` | Content Manager |

### User Accounts

| Username         | Password  | Name            |
| ---------------- | --------- | --------------- |
| `john_doe`       | `user123` | John Doe        |
| `jane_smith`     | `user123` | Jane Smith      |
| `alex_gamer`     | `user123` | Alex Wong       |
| `player_one`     | `user123` | Sam Lee         |
| `pro_gamer`      | `user123` | Chris Park      |
| `gamer_thai`     | `user123` | Somsak Jaidee   |
| `rpg_lover`      | `user123` | Wanida Sukjai   |
| `fps_king`       | `user123` | Arjun Patel     |
| `indie_fan`      | `user123` | Napat Wongkham  |
| `casual_play`    | `user123` | Pranee Chaichan |
| `space_gamer`    | `user123` | Alex Chen       |
| `night_hunter`   | `user123` | Sara Kim        |
| `pixel_master`   | `user123` | Mike Ross       |
| `loot_goblin`    | `user123` | Tanya Singh     |
| `open_world_fan` | `user123` | Leo Tanaka      |

---

## API Endpoints (Backend — Port 3000)

Base URL: `http://localhost:3000/api`

### Customer Authentication

| Method | Endpoint              | Description         |
| ------ | --------------------- | ------------------- |
| POST   | `/api/users/register` | Register a new user |
| POST   | `/api/users/login`    | User login          |
| GET    | `/api/users/:id`      | Get user profile    |

### Admin Authentication

| Method | Endpoint          | Description |
| ------ | ----------------- | ----------- |
| POST   | `/api/auth/login` | Admin login |

### Games (CRUD)

| Method | Endpoint         | Description                                |
| ------ | ---------------- | ------------------------------------------ |
| GET    | `/api/games`     | List/search games (supports query filters) |
| GET    | `/api/games/:id` | Get a single game by ID                    |
| POST   | `/api/games`     | Create a new game                          |
| PUT    | `/api/games/:id` | Update an existing game                    |
| DELETE | `/api/games/:id` | Delete a game                              |

**Supported search filters (query parameters):** `title`, `genre`, `minPrice`, `maxPrice`, `pricingType`

### Reviews

| Method | Endpoint                     | Description                |
| ------ | ---------------------------- | -------------------------- |
| GET    | `/api/reviews/:gameId`       | Get all reviews for a game |
| GET    | `/api/reviews/stats/:gameId` | Get rating statistics      |
| POST   | `/api/reviews`               | Submit a new review        |
| DELETE | `/api/reviews/:id`           | Delete a review            |

### Shopping Cart

| Method | Endpoint                        | Description                         |
| ------ | ------------------------------- | ----------------------------------- |
| GET    | `/api/cart/:sessionId`          | Get all cart items for a session    |
| GET    | `/api/cart/count/:sessionId`    | Get total item count in cart        |
| POST   | `/api/cart`                     | Add a game to cart (1 per game max) |
| PUT    | `/api/cart/:cartId`             | Update item quantity                |
| DELETE | `/api/cart/:cartId`             | Remove a single item from cart      |
| DELETE | `/api/cart/clear/:sessionId`    | Clear the entire cart               |

> **Cart Rule:** Each game can only be added **once per session**. Adding a duplicate returns `{ message: 'Already in cart' }` without creating a duplicate record.

### Wishlist

| Method | Endpoint                               | Description                     |
| ------ | -------------------------------------- | ------------------------------- |
| GET    | `/api/wishlist/:sessionId`             | Get all wishlisted games         |
| GET    | `/api/wishlist/check/:sessionId/:gId`  | Check if a game is wishlisted   |
| POST   | `/api/wishlist`                        | Add a game to wishlist          |
| DELETE | `/api/wishlist/:sessionId/:gameId`     | Remove a game from wishlist     |

### Gaming News (Public API Proxy)

| Method | Endpoint    | Description                           |
| ------ | ----------- | ------------------------------------- |
| GET    | `/api/news` | Fetch latest free-to-play game releases |

---

## Public Web Service Integration

The homepage integrates the **FreeToGame API** ([freetogame.com/api-doc](https://www.freetogame.com/api-doc)) to display the latest free-to-play game releases. This is a **fully free public API** that requires **no API key** and works out of the box with no additional configuration.

The backend acts as a proxy at `/api/news`, fetching data from FreeToGame and formatting it for the frontend. Gaming content loads automatically on the homepage news section.

---

## Database

The database (`game_store`) consists of **10 tables**:

| Table        | Description                                       |
| ------------ | ------------------------------------------------- |
| Administrator | Admin user profile information                   |
| AdminLogin   | Login credentials and login log                  |
| Game         | Game catalog with pricing and image data          |
| Genre        | Game genre categories                             |
| GameGenre    | Many-to-many game-to-genre junction table         |
| Review       | User-submitted game reviews and ratings           |
| Wishlist     | Session-based game wishlist entries               |
| AdminAddGame | Tracks which admin added each game                |
| User         | Customer accounts for registered users            |
| Cart         | Session-based shopping cart entries               |

---

## Core Features

- **Storefront Navigation** — Browse games by genre sidebar, search by title, and filter by price range.
- **Featured Game Banner** — Highlights the top game with a Buy Now button and wishlist shortcut.
- **Shopping Cart** — Add games (1 copy per game per session), view subtotal, discounts, and total. Cart badge updates in real time.
- **Wishlist** — Save games for later and move them to cart with one click.
- **User Authentication** — Tab-based Sign In and Create Account forms for customer accounts.
- **Admin Dashboard** — Full CRUD system to manage the game catalog, update pricing, and set games as Regular / Sale / Free.
- **Gaming News** — Real-time free-to-play game releases from the FreeToGame public API.
- **Image Gallery** — Detail page with an image slider and thumbnail navigation using locally hosted images.
- **Review System** — Submit ratings (1–5 stars) and comments, view review lists and star-distribution bar charts.
- **Team Page** — Real profile photos served from the local `/images/team/` directory.

---

## Tech Stack

| Layer      | Technology                      |
| ---------- | ------------------------------- |
| Frontend   | HTML5, CSS3, Vanilla JavaScript |
| Backend    | Node.js, Express.js             |
| Database   | MySQL                           |
| Auth       | Plain text password comparison  |
| CORS       | cors middleware                 |
| Env Config | dotenv                          |

---

## Remarks

- The frontend and backend run on **separate servers** — ports **5500** and **3000** respectively.
- CORS is enabled on the backend to allow cross-origin requests from the frontend server.
- All seed data (24 games, 28 genres, 10 admins, 41+ reviews, 10 users, 10 cart records, 10 wishlist entries) is defined in `sec2_gr12_database.sql` and imported automatically on the first run.
- Game images are **hosted locally** under `sec2_gr12_fe_src/images/games/<GameFolder>/` — `front.jpg` is the card cover image; `detail_N.jpg` files form the gallery slider on the detail page.
- Team profile photos are hosted locally at `sec2_gr12_fe_src/images/team/<studentId>.jpg`.
- The cart enforces a **1 copy per game** rule per session — adding the same game twice returns an `'Already in cart'` response without creating a duplicate.
- No Docker is required — the project connects directly to a local MySQL server.
- Clean URL routing is handled by Express Router on the frontend server. All old `.html` paths redirect to clean URLs automatically.
